...
class AzureClubMemberDataSource(private val database: Database) : ClubMemberDataSource {
    override suspend fun getClubsMembers(clubId: UUID): List<ClubMember> = newSuspendedTransaction(db = database) {
        ClubMembers.selectAll().where { ClubMembers.clubId eq clubId }
